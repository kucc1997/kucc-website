...
const boardData: Record<string, Executive[]> = {
  '2025/26': [
    {
      name: 'Sakshi K.C.',
...
export default function ExCommunity() {
  const [selectedBoard, setSelectedBoard] = useState<'2025/26' | '2025'>('2025/26')
  
  const executivesList = boardData[selectedBoard]
      ...
      <Card className="w-full border-2 shadow-lg bg-baseBackground px-4">
        <CardHeader className="flex items-center">
          <div className="flex items-center justify-center gap-4 mb-2">
            <CardDescription className="text-gray-500 text-lg">KUCC Board</CardDescription>
            <select
              value={selectedBoard}
              onChange={(e) => setSelectedBoard(e.target.value as '2025/26' | '2025')}
              className="bg-gray-900 border border-accentBlue/30 text-accentBlueLight rounded-lg px-4 py-2 text-sm font-medium hover:border-accentBlue/50 focus:outline-none focus:ring-2 focus:ring-accentBlue/50 transition-all cursor-pointer"
            >
